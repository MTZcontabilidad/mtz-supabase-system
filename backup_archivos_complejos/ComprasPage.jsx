...
const ComprasPage = () => {
  ...
  const categorias = ['Insumos', 'Equipos', 'Servicios', 'Materiales', 'Otros'];
  const estados = ['Pendiente', 'Aprobada', 'Rechazada', 'En proceso'];
  ...
  const cargarCompras = useCallback(async () => {
    try {
      ...
      const comprasPendientes = (data || []).filter(
        c => c.estado === 'Pendiente'
      ).length;
      const comprasAprobadas = (data || []).filter(
        c => c.estado === 'Aprobada'
      ).length;
      setStats({
  ...
  const comprasFiltradas = compras.filter(compra => {
    ...
    const matchesFilters =
      (!filters.categoria || compra.categoria === filters.categoria) &&
      (!filters.estado || compra.estado === filters.estado) &&
      (!filters.proveedor || compra.proveedor.includes(filters.proveedor));
