...
# Este script configura el token de acceso y verifica la conexión
...
Write-Host "📁 Verificando configuración del MCP..." -ForegroundColor Cyan
if (Test-Path ".cursor/mcp.json") {
    ...
    Write-Host "Configuración actual del MCP:" -ForegroundColor Yellow
    $mcpConfig | ConvertTo-Json -Depth 3 | Write-Host
} else {
    Write-Host "❌ Archivo .cursor/mcp.json no encontrado" -ForegroundColor Red
    Write-Host "Creando configuración del MCP..." -ForegroundColor Yellow

    # Crear configuración del MCP
    $mcpConfig = @{
        mcpServers = @{
            supabase = @{
                command = "npx"
    ...
    Write-Host "✅ Configuración del MCP creada" -ForegroundColor Green
}
...
VITE_APP_DESCRIPTION=Sistema de gestión empresarial MTZ
"@

    $envContent | Set-Content ".env.local"
    Write-Host "✅ Archivo .env.local creado" -ForegroundColor Green
    Write-Host "⚠️  IMPORTANTE: Actualiza VITE_SUPABASE_ANON_KEY con tu clave anónima" -ForegroundColor Yellow
}
...
# Verificar que el MCP de Supabase esté instalado
Write-Host "🔍 Verificando MCP de Supabase..." -ForegroundColor Cyan
try {
    $mcpVersion = npx @modelcontextprotocol/server-supabase --version 2>$null
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ MCP de Supabase instalado" -ForegroundColor Green
    } else {
        Write-Host "❌ MCP de Supabase no encontrado" -ForegroundColor Red
        Write-Host "Instalando MCP de Supabase..." -ForegroundColor Yellow
        npm install -g @modelcontextprotocol/server-supabase
    }
} catch {
    Write-Host "❌ Error verificando MCP de Supabase" -ForegroundColor Red
    Write-Host "Instalando MCP de Supabase..." -ForegroundColor Yellow
    npm install -g @modelcontextprotocol/server-supabase
}
...
try {
    ...
    Write-Host "✅ Conexión con Supabase exitosa" -ForegroundColor Green
} catch {
    Write-Host "❌ Error conectando con Supabase: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "Verifica que el token sea válido y el proyecto esté activo" -ForegroundColor Yellow
}
...
Write-Host "✅ MCP de Supabase instalado" -ForegroundColor White
Write-Host ""

Write-Host "🚀 PRÓXIMOS PASOS:" -ForegroundColor Yellow
Write-Host "1. Reinicia Cursor para que tome la nueva configuración del MCP" -ForegroundColor White
...
Write-Host "🎉 Configuración del MCP de Supabase completada!" -ForegroundColor Green
